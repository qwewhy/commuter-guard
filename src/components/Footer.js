...
const ChineseFlag = () => (
  ...
  >
    ...
    <g fill="#FFDE00">
      ...
      {/* Small stars */}
      <path
        d="M10.1294,2.5 l0.8939,2.75208 -2.3363,-1.69757 2.8853,0 -2.3363,1.69757z"
        transform="rotate(23.036243 10.1294 4.313657)"
      />
      <path
        d="M12.0589,4.55 l0.8939,2.75208 -2.3363,-1.69757 2.8853,0 -2.3363,1.69757z"
        transform="rotate(45.869898 12.0589 6.367937)"
      />
      <path
        d="M12.0589,7.5 l0.8939,2.75208 -2.3363,-1.69757 2.8853,0 -2.3363,1.69757z"
        transform="rotate(69.945396 12.0589 9.298907)"
      />
      <path
        d="M10.1294,9.5435 l0.8939,2.75208 -2.3363,-1.69757 2.8853,0 -2.3363,1.69757z"
        transform="rotate(20.659808 10.1294 11.419537)"
      />
    </g>
  </svg>
);
...
const Footer = () => {
  ...
  const selectedIndexRaw = menuItems.findIndex(
    (item) => item.path && location.pathname.startsWith(item.path)
  );
  const selectedIndex = selectedIndexRaw === -1 ? 0 : selectedIndexRaw;

  return (
    <footer className="bg-white text-black shadow-lg fixed bottom-0 left-0 right-0">
        <div className="container mx-auto px-4 py-4">
            <nav className="w-full relative">
                <div
                    className="absolute bottom-0 left-0 h-full bg-[#153FCE] rounded-lg transition-transform duration-300 z-0"
                    style={{
                            ...
                            <li key={index} className="flex-1 text-center">
                                {item.isDropdown ? (
                                    <button
                                        ...
                                        className={`transition duration-300 flex flex-col items-center py-2 w-full focus:outline-none ${moreButtonListIsOpen ? 'text-white' : 'text-black hover:text-[#153FCE]'}`}
                                    >
                                        {item.icon}
                                    ...
                                    <Link
                                        to={item.path}
                                        className={`transition duration-300 flex flex-col items-center py-2 ${isSelected ? 'text-white' : 'text-black hover:text-[#153FCE]'}`}
                                    >
                                        {item.icon}
                                        <span className="text-xs">{item.label}</span>
                                    </Link>
                ...
                {/* The dropdown menu for 'more' */}
                {moreButtonListIsOpen && (
                    <div className="absolute right-0 bottom-full mb-2 w-48 bg-white rounded-md shadow-lg py-1 z-10" ref={dropdownRef}>
